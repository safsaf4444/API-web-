...
from typing import AsyncGenerator  # List removed — unused (Issue 3)
...
def _parse_clinical_cache(cached: AIResult) -> tuple[dict, dict, dict, list, list]:
    """
    Issue 2 fix: fully parse all fields from a cached clinical AIResult.
    ...
    try:
        ...
        if "pico" in raw_q:
            # v3.1 format — full cache_data stored
            ...
            # v3.0 format — only pico stored in question
            pico_raw = raw_q or {}
        if not stats_raw:
...
async def ai_clinical(payload: AIClinicalRequest, session: Session = Depends(get_session), current_user: User = Depends(get_current_user)):
    ...
    # Issue 2 fix: fully restore all fields from cache, not just pico+stats
...
def get_clinical(study_id: int, session: Session = Depends(get_session), current_user: User = Depends(get_current_user)):
    ...
    return AIClinicalResponse(
        ...
        jargon=jargon_items,
        cached=True,
        prompt_version=cached.prompt_version if cached else None,
    )
...
def get_shared_evidence(token: str, session: Session = Depends(get_session)):
    ...
    # Issue 6 fix: targeted query instead of loading all owner studies
    # Match by cache_key directly using a subquery approach — avoid O(n) scan
    title    = "Untitled"
    study_id = None
    owner_studies = session.exec(
        select(Study).where(Study.owner_username == cached.owner_username).limit(500)
    ).all()
    for s in owner_studies:
        if _cache_key("clinical", s.title, s.doi, s.pmid, s.pmcid) == cached.cache_key:
            title    = s.title
            study_id = s.id
            break  # already present but made explicit
...
def _build_weighting(studies):
    ...
    for s in studies:
        ...
        weights.append({
            ...
            "year":        s.year,
            "base_score":  EVIDENCE_WEIGHTS.get((s.study_type or "other").lower().replace(" ", "_"), 1.0),
...
def _build_synthesis_response(rec, study_ids, cached):
    ...
    return AISynthesisResponse(
        synthesis_id=rec.id or 0,
        study_ids=study_ids,
        paper_count=rec.paper_count,
        synthesis_narrative=rec.synthesis_narrative,
        consensus_points=consensus,
        contradictions=contradictions,
        gap_analysis=gaps if isinstance(gaps, list) else [],
        weighted_conclusion=rec.weighted_conclusion,
        steel_man=rec.steel_man,
        comparative_methodology=rec.comparative_methodology,
        weighting_breakdown=weighting,
        cached=cached,
...
async def ai_synthesise(payload: AISynthesisRequest, session: Session = Depends(get_session), current_user: User = Depends(get_current_user)):
    ...
    try:
        rec = SynthesisResult(
            ...
            prompt_version="4.0", model_used=result.provider.value, paper_count=len(studies),
        )
        session.add(rec)
        session.commit()
        session.refresh(rec)
...
async def ai_subject_query(payload: AISubjectQueryRequest, session: Session = Depends(get_session), current_user: User = Depends(get_current_user)):
    ...
    try:
        rec = SynthesisResult(
            ...
            prompt_version="4.0", model_used=result.provider.value, paper_count=len(papers_to_use),
        )
        session.add(rec)
        session.commit()
        session.refresh(rec)
    ...
    return AISubjectQueryResponse(
        synthesis_id=synthesis_id or 0,
        query=payload.query,
        papers_found=len(papers_to_use),
        synthesis_narrative=data.get("synthesis_narrative"),
        consensus_points=consensus,
        contradictions=contradictions,
